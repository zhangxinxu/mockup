...
let qCss = function (src, dist) {
	...
	fs.readdirSync(src).forEach(function (filename) {
		...
		if (/\.qcss$/.test(filename) && /^_/.test(filename) == false) {
			// .qcss文件才处理
			// 读文件内容
			var data = fs.readFileSync(pathQcssFile, {
				// 需要指定编码方式，否则返回原生buffer
				encoding: 'utf8'
			});
			...
			data = data.replace(/@import\s+([\w\W]*?);/g, function (matchs, url) {
				if (/\.qcss/.test(url)) {
					...
					// 判断qcss文件是否有
					var pathImportQcss = path.join(src, url);

					if (!fs.existsSync(pathImportQcss)) {
						console.error(pathImportQcss + '文件不存在，忽略这段引入');
						return '/* '+ matchs +' */';
					}
					// 替换成对应文件内容
					return fs.readFileSync(pathImportQcss);
				}
				// 非.qcss格式原路返回
				return matchs;
			});
